 ...
 */
  ...
  static get DATABASE_URL() {
    ...
    return `http://localhost:${port}/data/bettas.json`;
  }
  ...
  */
  ...
  return (`/img/${betta.photograph}-300_2x.jpg`);
  }
  /**
   * Map marker for a restaurant.
   */
   static mapMarkerForRestaurant(betta, map) {
    // https://leafletjs.com/reference-1.3.0.html#marker  
    const marker = new L.marker([betta.latlng.lat, betta.latlng.lng],
      {title: betta.name,
      alt: betta.name,
      url: DBHelper.urlForRestaurant(betta)
      })
      marker.addTo(newMap);
    return marker;
  } 
